  ...
  return (
    <div className="relative flex min-h-[100dvh] w-full items-center justify-center bg-navy-950 px-6 text-white">
      <div
        ...
        style={{ background: "radial-gradient(circle, rgba(1,95,255,0.32), transparent 70%)" }}
      />
      <div className="relative w-full max-w-[380px]">
        ...
        <h1 className="text-[1.6rem] font-black leading-tight tracking-tight text-white md:text-[1.9rem]">
          Agencilab Sales Bible
        </h1>
        <p className="mt-2.5 text-[0.92rem] leading-relaxed text-white/55">
          La bible interne des closers. Saisis le mot de passe.
        </p>
        ...
        <form action={formAction} className="mt-7 flex flex-col gap-3">
          ...
          />
          {state.error && (
            <p className="text-[0.85rem] font-medium text-red-400">{state.error}</p>
          )}
          <button
            type="submit"
            disabled={pending}
          ...
          >
            {pending ? "Vérification..." : "Ouvrir la bible"}
          </button>
        </form>
      </div>
    </div>
